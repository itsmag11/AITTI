...
from diffusers.utils import deprecate, logging, replace_example_docstring, scale_lora_layers, unscale_lora_layers
...
class StableDiffusionAdaptiveTokenPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin, FromSingleFileMixin):
    ...
    def get_text_embeddings(self, input_ids, hidden_states,
                            attention_mask = None,
    ...
    def encode_prompt(
        ...
        print(f'prompt pair: {prompt}', flush=True)
        ...
        if prompt_embeds is None:
          ...
          ## ---------------------------------------------------------------------------------------------------------
            ...
            for profession_word in profession_name.split(' '):
                concept_mask[prompt[0].split(' ').index(profession_word) + 1] = True
            ## ---------------------------------------------------------------------------------------------------------
            ...
            if clip_skip is None:
                prompt_embeds_ori = self.text_encoder(text_input_ids.to(device), attention_mask=attention_mask)
                prompt_embeds_ori = prompt_embeds_ori[0]

                token_embeddings = self.get_lookup_embeddings(text_input_ids.to(device)) ## torch.Size([1, 77, 768])
                ...
                token_embeddings_ori = token_embeddings.clone()

                print(f'=== adaptive token mapping ===', flush=True)

                placeholders_token_embeddings = token_embeddings[0][placeholders_mask] ## N X 768
                concept_token_embeddings = token_embeddings[0][concept_mask]
                input_token_embeddings = torch.cat([placeholders_token_embeddings, concept_token_embeddings], dim=0)
                adaptive_output = self.adaptive_mapping(input_token_embeddings) ## 768
                placeholders_num = placeholders_token_embeddings.shape[0]
                adaptive_tokens = adaptive_output[:placeholders_num].to(dtype=prompt_embeds_dtype, device=device)
                token_embeddings[0][placeholders_mask] = adaptive_tokens.unsqueeze(0)

                assert (token_embeddings[0][placeholders_mask] == token_embeddings_ori[0][placeholders_mask]).all() == False
                assert (token_embeddings[0][~placeholders_mask] == token_embeddings_ori[0][~placeholders_mask]).all() == True
                assert (token_embeddings[1] == token_embeddings_ori[1]).all() == True

                ## ---------------------------------------------------------------------------------------------------------
                
                token_embeddings_w_pos = self.add_position_embeddings(text_input_ids.to(device), token_embeddings)
                prompt_embeds = self.get_text_embeddings(text_input_ids.to(device), token_embeddings_w_pos, attention_mask=attention_mask)
